...
public class EnemyController : MonoBehaviour {
	...
	void Patrol() {
		...
		//             how many seconds he will patrol before we set new random destination
		...
		if (Vector3.Distance(transform.position, target.position) <= chase_Distance) {
			...
			enemy_Audio.Play_ScreamSound(); // play spotted audio
	...
	void Chase() {
		...
		// because we are chasing (tunning towards) the player
		...
		} // if the distance between enemy and player is greater than chase distance
	...
	void Attack() {
		...
		if (attack_Timer > wait_Before_Attack) {
			...
			enemy_Audio.Play_AttackSound(); // play attack sound
		}
